...
class Miner
    ...
    response = self.class.get("/addMinedBlock?nonce=#{mined_block[:nonce]}&miningRewardAddress=#{@address}")
	...
	private
      ...
      blockinfo = "#{block[:previousHash]}#{block[:transactions].to_json}#{block[:nonce]}"
			Digest::SHA256.hexdigest("#{blockinfo}")
		end
end
